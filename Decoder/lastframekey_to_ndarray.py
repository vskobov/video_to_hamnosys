...
def lastframe_handle(frame_name, path, plot = False, text_out = False):
    #lastframe_keys = glob.glob(path+'/*.json')[0]
    lastframe_keys = glob.glob(path+'keys_'+frame_name+'/4_*.json')[0]

    #print(lastframe_keys)
    ...
    with open(lastframe_keys) as json_file:
   ...
   # np.save(path+'input_keys_2d_np_r_hand_location_'+frame_name, np_r_hand_location_matrix)
    ...
    np.save(path+'lf_input_keys_1d_np_all_'+frame_name, concat_to_one_dim(np_movement_matrix))
...
#    np.save(path+'input_keys_1d_np_r_hand_location_'+frame_name, concat_to_one_dim(np_r_hand_location_matrix))
    ...
    np.save(path+'lf_input_keys_1d_np_lr_hand_conf_'+frame_name, np.concatenate((concat_to_one_dim(np_l_hand_conf_matrix), concat_to_one_dim(np_r_hand_conf_matrix)), axis=0) )
    np.save(path+'lf_input_keys_1d_np_lr_hand_orient_'+frame_name, np.concatenate((concat_to_one_dim(np_l_hand_orient_matrix), concat_to_one_dim(np_r_hand_orient_matrix)), axis=0) )
    np.save(path+'lf_input_keys_1d_np_lr_hand_location_'+frame_name, np.concatenate((concat_to_one_dim(np_l_hand_location_matrix), concat_to_one_dim(np_r_hand_location_matrix)), axis=0) )
...
for d in dirs:
    ...
    try:
        lastframe_handle(p.parts[-1],d,False,False)
        #print(p.parts[-1])
        #print(d)
        #print(glob.glob(d+'keys_'+p.parts[-1]+'/4_*.json')[0])
    except:
        print('Missed '+ d)
