...
for d in range(0,len(dirs)):
    p = pathlib.PurePath(dirs[d])
    if d == 0:
        create_h5(path+"lf_test_input_keys_1d_np_lr_hand_conf_h5.hdf5",(dirs[d]+'lf_input_keys_1d_np_lr_hand_conf_'+p.parts[-1]+'.npy'),(dirs[d]+'input_vector_h_conf_'+p.parts[-1]+'.npy'))
    if d < VALID_SET and d > 0 :
        h5py_data_appender(path+"lf_test_input_keys_1d_np_lr_hand_conf_h5.hdf5",(dirs[d]+'lf_input_keys_1d_np_lr_hand_conf_'+p.parts[-1]+'.npy'),(dirs[d]+'input_vector_h_conf_'+p.parts[-1]+'.npy'),True)
    if d==VALID_SET:
